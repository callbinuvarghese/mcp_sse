...
class WeatherExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        logger.debug('Processing request with new message: %s', new_message)
        ...
        try:
            logger.debug('Running ADK runner with session_id: %s', session_id)
    ...
    async def execute(
        ...
        logger.debug(f'[weather] execute called with context: %s', context)
        ...
        logger.debug('[weather] execute over')
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        ...
        """
        logger.debug(f'Cancel called with context: {context}')
    ...
    async def _upsert_session(self, session_id: str) -> 'Session':
        ...
        """
        logger.debug(f'Upserting session with ID: {session_id}')
