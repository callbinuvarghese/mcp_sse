...
def create_send_message_payload(
    ...
    if task_id:
        logger.debug(f'Adding taskId: {task_id} to payload')
...
# ) -> dict[str, Any]:
#     """Callback function to send a task to a remote agent."""
#     logger.debug(f'Sending task to agent: {agent_name} with task: {task}')
#     if agent_name not in tool_context.state['remote_agent_connections']:
#         logger.error(f'Agent {agent_name} not found')
#         raise ValueError(f'Agent {agent_name} not found')
    
#     state = tool_context.state
#     state['active_agent'] = agent_name
#     client = tool_context.state['remote_agent_connections'][agent_name]

#     if not client:
#         logger.error(f'Client not available for {agent_name}')
...
#     logger.debug(
#         f'Sending message to agent {agent_name} with payload: {message_request.model_dump_json(exclude_none=True, indent=2)}'
#     )
    
#     send_response: SendMessageResponse = asyncio.run(
#         client.send_message(message_request=message_request)
#     )

#     logger.debug(
#         f'Message response : {send_response.model_dump_json(exclude_none=True, indent=2)}'
#     )
...
#     logger.debug(
#         f'Received task response from agent {agent_name}: {send_response.root.result.model_dump(exclude_none=True)}'
#     )
...
class RoutingAgent:
    ...
    async def _async_init_components(
        ...
        logger.debug(
            f'Initializing RoutingAgent with remote agent addresses: {remote_agent_addresses}'
        )
        async with httpx.AsyncClient(timeout=30) as client:
            for address in remote_agent_addresses:
                ...
                try:
                    ...
                    logger.debug(
                        f'Resolved agent card: {card.model_dump(exclude_none=True)}'
                    )
                    remote_connection = RemoteAgentConnections(
                        agent_card=card, agent_url=address
                    )
                    logger.debug(
                        f'Initialized RemoteAgentConnections for {card.name} at {address}'
                    )
        ...
        for agent_detail_dict in self.list_remote_agents():
            logger.info(f'Adding Remote Agents detail dict: {agent_detail_dict}')
    ...
    async def create(
        ...
        if task_callback is None:
            logger.debug('No task_callback provided, using default.')
        else:
            logger.debug('task_callback class/method provided')
        instance = cls(task_callback)
        await instance._async_init_components(remote_agent_addresses)
        logger.debug(
            f'Initialized RoutingAgent with remote agents: {instance.list_remote_agents()}'
        )
        logger.debug(f'RoutingAgent created successfully {instance.__class__}')
         # Return the instance
        return instance
    ...
    def before_model_callback(
        ...
        logger.debug(
            f'Before model callback with context'
        )
    ...
    async def send_message(
        ...
        if 'task_id' in state:
            ...
            logger.debug(f'Adding existing taskId: {task_id} from state to payload')
        else:
            logger.debug('No existing taskId found, generating a new one')
            #task_id=str(uuid.uuid4())

        if 'context_id' in state:
            context_id = state['context_id']
            logger.debug(f'Adding existing context_id: {context_id} from state to payload')
        else:
            logger.debug('No existing context_id found, generating a new one')
        ...
        if task_id:
            ...
            logger.debug('No task_id provided, not adding to payload')
...
def _get_initialized_routing_agent_sync() -> Agent:
    ...
    async def _async_main() -> Agent:
        ...
        logger.debug(
            f'Initialized RoutingAgent with remote agents: {routing_agent_instance.list_remote_agents()}'
        )
    ...
    try:
        ...
        if 'asyncio.run() cannot be called from a running event loop' in str(e):
            logger.warning(
                f'Could not initialize RoutingAgent with asyncio.run(): {e}. '
