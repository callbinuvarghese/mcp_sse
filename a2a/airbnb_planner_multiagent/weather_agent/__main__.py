...
from request_logger import RequestLogMiddleware
...
def main(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    ...
    logger.debug(f'Creating agent card with skill: {skill}')
    ...
    logger.debug(f'ADK agent created: {adk_agent}')
    runner = Runner(
    ...
    logger.debug(f'Agent executor created: {agent_executor}')
    ...
    app = a2a_app.build()
    app.add_middleware(RequestLogMiddleware)   # <-- add here
    logger.debug('A2A Starlette application created')
    uvicorn.run(a2a_app.build(), host=host, port=port, log_level="debug",access_log=True)
