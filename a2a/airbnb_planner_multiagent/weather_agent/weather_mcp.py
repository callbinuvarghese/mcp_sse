...
async def get_weather_response(endpoint: str) -> dict[str, Any] | None:
    ...
    """
    try:
        # Use the shared http_client to make the request
        logger.debug(f'Making request to NWS API endpoint: {endpoint}')
...
async def get_alerts(state: str) -> str:
    ...
    """
    logger.debug(f'Fetching weather alerts for state: {state}')
...
async def get_forecast(latitude: float, longitude: float) -> str:
    ...
    """
    logger.debug(f'Fetching weather forecast for coordinates: {latitude}, {longitude}')
    # Input validation
...
async def get_forecast_by_city(city: str, state: str) -> str:
    ...
    """
    logger.debug(f'Fetching weather forecast for city: {city}, state: {state}')
    # --- Input Validation ---
