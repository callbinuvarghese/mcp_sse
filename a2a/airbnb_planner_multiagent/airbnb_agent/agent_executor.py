...
class AirbnbAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        logger.debug('Executing AirbnbAgentExecutor with context: %s', context)
        ...
        async for event in self.agent.stream(query, task.context_id):
            if event['is_task_complete']:
                logger.debug('Task is complete, processing final response.')
                ...
                logger.debug('User input required, updating task status.')
                await event_queue.enqueue_event(
                    TaskStatusUpdateEvent(
                        status=TaskStatus(
                ...
                logger.debug('Updating task status to working.')
                await event_queue.enqueue_event(
                    TaskStatusUpdateEvent(
                        status=TaskStatus(
